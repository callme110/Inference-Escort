...
from inference_escort.exception import InferenceEscortValidationError
from inference_escort.model import Model
from inference_escort.transformers_helpers import get_tokenizer_and_model_for_classification, pipeline
from inference_escort.util import calculate_risk_score, get_logger
...
class Code(Scanner):
    ...
    def __init__(
        ...
        *,
        ...
        Raises:
            InferenceEscortValidationError: If the languages are not a subset of SUPPORTED_LANGUAGES.
        """
        if not set(languages).issubset(set(SUPPORTED_LANGUAGES)):
            raise InferenceEscortValidationError(f"Languages must be a subset of {SUPPORTED_LANGUAGES}")
