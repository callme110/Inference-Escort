...
from .exception import InferenceEscortValidationError
...
def is_onnx_supported() -> bool:
    ...
    if not is_supported:
        LOGGER.warning(
            ...
            "`pip install inference-escort[onnxruntime]` for CPU or "
            "`pip install inference-escort[onnxruntime-gpu]` for GPU to enable ONNX Runtime optimizations."
        )
...
def pipeline(
    ...
    **kwargs,
    ...
    if task not in get_args(ClassificationTask):
        raise InferenceEscortValidationError(f"Invalid task. Must be one of {ClassificationTask}")
