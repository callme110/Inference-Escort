...
1. Copy the code from [inference_escort_api](https://github.com/callme110/Inference-Escort/tree/main/inference_escort_api)
...
Either build the Docker image or pull our official image from [Docker Hub](https://hub.docker.com/r/callme110/inference-escort-api).
...
```bash
docker pull callme110/inference-escort-api:latest
```

Now, you can run the Docker container:

```bash
docker run -d -p 8000:8000 -e LOG_LEVEL='DEBUG' -e AUTH_TOKEN='my-token' callme110/inference-escort-api:latest
```
...
```bash
docker run -d -p 8000:8000 -e APP_WORKERS=1 -e AUTH_TOKEN='my-token' -e LOG_LEVEL='DEBUG' -v ./entrypoint.sh:/home/user/app/entrypoint.sh -v ./config/scanners.yml:/home/user/app/config/scanners.yml callme110/inference-escort-api:latest
```
