# Apply Inference-Escort Content Mod across 100+ LLMs w/ LiteLLM

Use Inference-Escort with LiteLLM Proxy to moderate calls across Anthropic/Bedrock/Gemini/etc. LLMs with [LiteLLM](https://github.com/BerriAI/litellm)
...
- Setup [Inference-Escort Docker](../api/deployment.md#from-docker)

## Quick Start

Let's add Inference-Escort content mod for Anthropic API calls

Set the Inference-Escort API Base in your environment
...
- Check your proxy logs for any statement with `Inference-Escort:`

Expected results:

```bash
Inference-Escort: Received response - {"sanitized_prompt": "hello world", "is_valid": true, "scanners": { "Regex": 0.0 }}
```
### Turn on/off per key
...
litellm_settings:
    callbacks: ["llmguard_moderations"]
    inference_escort_mode: "key-specific"
...
--data '{
    ...
    "permissions": {
        "enable_inference_escort_check": true # 👈 KEY CHANGE
    }
}'
...
litellm_settings:
    callbacks: ["llmguard_moderations"]
    inference_escort_mode: "request-specific"
```
