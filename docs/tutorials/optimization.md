...
```sh
pip install inference-escort[onnxruntime] # for CPU instances
pip install inference-escort[onnxruntime-gpu] # for GPU instances
```
...
```python
from inference_escort.input_scanners.code import Code, DEFAULT_MODEL
...
To optimize the output scanning, you can analyze the output in chunks. In [OpenAI](./openai.md) guide, we demonstrate how to use Inference-Escort to protect OpenAI client with streaming.
