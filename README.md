...
Inference-Escort 的核心功能是为大语言模型应用提供一套可插拔的安全检测、文本清洗和风险评估机制。它既可以作为 Python 库嵌入业务代码，也可以通过 API 服务独立运行在模型调用链旁边。直观地说，它不是替代 LLM 的“大脑”，而是陪同请求进入和离开模型的一组“安全检查员”：输入进入模型前先被检查，模型输出返回用户前再被复核。

从功能模型上看，项目将每一次检测抽象成一个 scanner：

\[
S_i(x) = (x_i', v_i, r_i)
\]

其中，\(x\) 表示原始文本，\(x_i'\) 表示 scanner 处理后的文本，\(v_i \in \{\text{True}, \text{False}\}\) 表示是否通过检查，\(r_i \in [0,1]\) 表示风险分数。多个 scanner 串联后，就形成一条安全流水线：

\[
x \rightarrow S_1 \rightarrow S_2 \rightarrow \cdots \rightarrow S_n \rightarrow (x', V, R)
\]

这意味着项目并不是只做“发现问题”，还可以在部分场景下对文本进行“修复或变换”，例如脱敏、截断、删除不可见字符、修复 JSON、恢复脱敏占位符等。

项目主要包含以下功能：

1. 输入安全扫描  
   在用户输入进入模型之前，项目可以检测 prompt injection、敏感信息、密钥泄露、超长 token、不可见字符、禁止关键词、禁止主题、代码片段、异常语言、毒性内容和情绪倾向等问题。它解决的是“危险输入是否应该进入模型”的问题。

2. 输出安全扫描  
   在模型生成结果返回用户或下游系统之前，项目可以检测敏感信息泄露、恶意链接、JSON 格式错误、模型拒答、回答相关性、事实一致性、偏见、毒性、乱码、语言不一致和输出过长等问题。它解决的是“模型回答是否可以被交付”的问题。

3. 文本脱敏与反脱敏  
   项目支持在输入阶段识别姓名、邮箱、电话、IP、信用卡等隐私实体，并将其替换为占位符；同时通过 `Vault` 保存映射关系，在输出阶段根据需要恢复原始信息。可以把 `Vault` 理解成一张临时的“匿名化对照表”，它让模型尽量少接触真实敏感数据，同时又不破坏业务流程。

4. 风险评分与放行决策  
   每个 scanner 都会返回 `is_valid` 和 `risk_score`。前者用于判断是否通过，后者用于表达风险程度。业务系统可以根据这些结果选择直接放行、拒绝请求、要求用户重写、进入人工审核，或者仅记录风险日志。

5. 可插拔扫描流水线  
   输入扫描器和输出扫描器都以列表形式组合。开发者可以根据任务类型选择不同策略，例如客服系统更关注隐私泄露和毒性内容，代码助手更关注代码安全和密钥泄露，RAG 系统更关注 prompt injection、事实一致性和输出相关性。

6. API 服务化能力  
   除了直接在 Python 代码中调用 `scan_prompt` 和 `scan_output`，项目还提供 FastAPI 服务层，可以将扫描能力作为独立安全服务供其他系统调用。API 模式适合多个应用共享同一套安全策略，也便于统一配置、监控和扩展。

7. 配置化与扩展能力  
   项目内置多种 scanner，同时保留自定义 scanner 的扩展方式。开发者可以通过继承基础 scanner 接口，把新的检测逻辑接入同一条流水线。换句话说，它的设计重点不是单个规则有多“万能”，而是让不同规则可以像积木一样组合起来，形成符合具体业务场景的安全策略。

## 项目定位

仓库将 Inference-Escort 描述为 `The Security Toolkit for LLM Interactions`。从代码角度看，它的核心并不是“再造一个模型”，而是给已有模型调用增加一层安全治理。
...
- 项目仓库：https://github.com/your-org/inference-escort
- 项目文档：https://your-org.github.io/inference-escort/
- API 文档：https://your-org.github.io/inference-escort/api/overview/
