...
# Install Inference-Escort reference https://github.com/GoogleCloudPlatform/generative-ai/blob/main/gemini/getting-started/intro_gemini_python.ipynb for more detailed instructions
...
# Import Scanners from Inference-Escort
from inference_escort import scan_output, scan_prompt
from inference_escort.input_scanners import PromptInjection, TokenLimit, Toxicity
from inference_escort.output_scanners import NoRefusal, Relevance, Sensitive

# Give a prompt
prompt = """ #What is Sundar's email?#"""

# Set your values for Inference-Escorts Input Scanners, this example uses the defaults
input_scanners = [TokenLimit(), Toxicity(), PromptInjection()]

# Set response variables
sanitized_prompt, results_valid, results_score = scan_prompt(input_scanners, prompt)

# Set Values for Inference-Escorts Output Scanners, this example uses the defaults
...
if all(results_valid.values()) is True:
    ...
    if not all(output_results_valid.values()) is True:
        # If the models response is above the Inference-Escortrules output scanners threshold then print the Prompt and the message "Prompt is not valid"
