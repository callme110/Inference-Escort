...
try:
    import inference_escort
except ImportError:
    raise ModuleNotFoundError(
        "Could not import inference-escort python package. Please install it with `pip install inference-escort`."
    )


class InferenceEscortPromptException(Exception):
    """Exception to raise when inference-escort marks prompt invalid."""


class InferenceEscortPromptChain(Chain):
    scanners: Dict[str, Dict] = {}
    """The scanners to use."""
    scanners_ignore_errors: List[str] = []
    """The scanners to ignore if they throw errors."""
    vault: Optional[inference_escort.vault.Vault] = None
    """The scanners to ignore errors from."""
    raise_error: bool = True
    """Whether to raise an error if the InferenceEscort marks the prompt invalid."""
    ...
    def init_scanners(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Raises:
            ValueError: If there is an issue importing 'inference-escort' or loading scanners.
        """
            ...
            if values.get("scanners") is not None:
                ...
                for scanner_name in values.get("scanners"):
                    ...
                    values["initialized_scanners"].append(
                        inference_escort.input_scanners.get_scanner_by_name(scanner_name, scanner_config)
                    )
    ...
    def _check_result(
        ...
        if self.raise_error:
            raise InferenceEscortPromptException(
    ...
    def _call(
        ...
        Raises:
            InferenceEscortPromptException: If there is an error during the scanning process
        """
        if run_manager:
            run_manager.on_text("Running InferenceEscortPromptChain...\n")
...
class InferenceEscortOutputException(Exception):
    """Exception to raise when inference-escort marks output invalid."""


class InferenceEscortOutputChain(BaseModel):
    class Config:
    ...
    """The scanners to ignore errors from."""
    raise_error: bool = True
    """Whether to raise an error if the InferenceEscort marks the output invalid."""
        ...
        try:
            if values.get("scanners") is not None:
                ...
                for scanner_name in values.get("scanners"):
                    ...
                    values["initialized_scanners"].append(
                        inference_escort.output_scanners.get_scanner_by_name(scanner_name, scanner_config)
                    )
    ...
    def _check_result(
        ...
        if self.raise_error:
            raise InferenceEscortOutputException(
...
vault = inference_escort.vault.Vault()
use_onnx = True

inference_escort_prompt_scanner = InferenceEscortPromptChain(
    vault=vault,
    scanners={
...
inference_escort_output_scanner = InferenceEscortOutputChain(
    vault=vault,
    scanners={
        "BanSubstrings": {
            "substrings": ["Laiyer"],
...
guarded_chain = (
    inference_escort_prompt_scanner  # scan input here
    | prompt
    | llm
    | (
        lambda ai_message: inference_escort_output_scanner.scan(input_prompt, ai_message)
